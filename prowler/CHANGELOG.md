...
- M365 `entra_users_mfa_capable` and `entra_break_glass_account_fido2_security_key_registered` now emit a single FAIL with a clear permission error instead of silent false positives for all users when `AuditLog.Read.All` is missing
